...
- This is an explicit full transcript fetch using the existing guarded `content_full=1` path; it is never automatic polling. Simultaneous expansion actions share one request per session and navigation generation, and the request state is released on failure or success.
