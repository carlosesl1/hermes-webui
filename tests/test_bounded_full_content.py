...
def test_full_content_expansion_without_older_rows(clipped_owner):
    ...
    functions = []
    for name in ["function _syncToolCallsForLoadedMessages(", "async function _ensureAllMessagesLoaded(", "async function expandFullTranscript("]:
...
let _messagesTruncated=false,_loadingOlder=false,_loadingSessionId=null,_oldestIdx=0,requests=0,renders=0;
