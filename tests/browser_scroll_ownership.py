...
BASE_UI = subprocess.check_output(['git', 'show', BASELINE + ':static/ui.js'], cwd=ROOT) if BASELINE else None
...
def main():
                ...
                for virtual in [False, True]:
                    ...
                    def route(request):
                        ...
                        if path == '/':
                            ...
                            request.fulfill(body=BASE_UI if path == '/static/ui.js' and BASE_UI is not None else file.read_bytes(), content_type=mimetypes.guess_type(file)[0] or 'application/octet-stream')
                    ...
                    page.evaluate(r"""({mode, virtual}) => {
                        ...
                        S.session={session_id:'scroll-A',tool_calls:[]}; S.busy=false;
                    ...
                    result = page.evaluate("""() => {
                        const e=$('messages'); const before=e.scrollTop;
                        renderMessages({preserveScroll:true});
                        const renderDelta=e.scrollTop-before;
                        ...
                        return {renderDelta, staleSessionDelta:e.scrollTop-target, virtualSpacers:document.querySelectorAll('[data-virtual-spacer]').length, messages:S.messages.length};
                    }""")
                    ...
                    flow = page.evaluate(r"""async () => {
                        ...
                        const snap=()=>_captureMessageViewportAnchor();
                        const delta=a=>{
                            if(!a) return null;
                        ...
                        const a=snap();
                        const savedApi=api;
                        _messagesTruncated=true; _oldestIdx=20;
                        api=async()=>({session:{session_id:'scroll-A',_messages_offset:0,_messages_truncated:false,tool_calls:[],messages:Array.from({length:20},(_,i)=>({role:i%2?'assistant':'user',id:'older'+i,content:'Older turn '+i+' '+('history '.repeat(35))}))}});
                        try { await _loadOlderMessages(); } finally {api=savedApi;}
                        await new Promise(r=>setTimeout(r,150));
                        const prependDelta=delta(a);
                        ...
                        return {prependDelta,streamDelta,settleDelta,readingOlder,tailDistance,loadedCount:S.messages.length,
    ...
    failures=[r for r in results if
        any(r[k] is None or abs(r[k])>2 for k in ['renderDelta','staleSessionDelta','postProcessDelta','prependDelta','streamDelta','settleDelta'])
        or bool(r['virtualSpacers']) != r['virtual'] or not r['readingOlder']
        or r['tailDistance']>2 or r['loadedCount'] != 202 or r['bodyOverflow']>2 or r['paneOverflow']>2]
